...
namespace SolidEdgeAdd_In.Helpers
{
    public class SaveAsStepHelper
    {
        public static string GetPath(SeDocument document)
        {
            ...
            using var properties = new PropertyProvider(document); string name = $"{properties.MaterialName}_{properties.Count}szt_{properties.Material}_{Path.GetFileNameWithoutExtension(document.FullName)}.step";
        ...
        public static void Save(SeDocument document, bool isConfirmed, string path) { if (!isConfirmed || string.IsNullOrEmpty(path)) return; document.SaveAs(path); }
    }
}