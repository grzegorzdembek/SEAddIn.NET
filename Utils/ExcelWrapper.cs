namespace SolidEdgeAdd_In.Utils
{
    public class ExcelWrapper
    {
        public static int GetColumnNumber(ExcelRange range, string name)
        {
            ...
            try
            {
                ...
                if (rowData != null)
                {
                    ...
                    for (int i = 1; i <= colCount; i++) { if (rowData[1, i]?.ToString() == name) return i; }
                }
        ...
        {
            ExcelStyles styles = null; ExcelStyle assemblyStyle = null; ExcelStyle partStyle = null; ExcelStyle steelmakingStyle = null; ExcelStyle boughtStyle = null; ExcelStyle normalStyle = null; ExcelStyle sheetMetalStyle = null;
            ...
            {
                CoreUtils.ReleaseCom(ref sheetMetalStyle); CoreUtils.ReleaseCom(ref normalStyle); CoreUtils.ReleaseCom(ref boughtStyle); CoreUtils.ReleaseCom(ref steelmakingStyle); CoreUtils.ReleaseCom(ref partStyle); CoreUtils.ReleaseCom(ref assemblyStyle); CoreUtils.ReleaseCom(ref styles);
            }
        }
        ...
        public static void TypeMemory(object[,] data, int typeNumber, int rowCount)
        {
            ...
            {
                object rawValue = data[i, typeNumber]; if (rawValue == null) continue;
        ...
        {
            ExcelRange range = null; ExcelRange dataRange = null; ExcelRange columns = null; ExcelRange anchorCell = null; ExcelFormatConditions conditions = null;

            try
            {
                range = worksheet.UsedRange; int lastRow = range.Rows.Count; int lastCol = range.Columns.Count;

                if (lastRow < 2) return;

                dataRange = range.Range[range.Cells[2, 1], range.Cells[lastRow, lastCol]]; columns = range.Columns;

                anchorCell = worksheet.Cells[2, typeNumber]; string address = anchorCell.Address[RowAbsolute: false, ColumnAbsolute: true];
        ...
        public static void CountMemory(object[,] data, int multiplier, int countNumber, int rowCount)
        {
            for (int i = 3; i <= rowCount; i++)
            {
                ...
                if (rawValue != null && int.TryParse(rawValue.ToString(), out int currentQty)) { data[i, countNumber] = currentQty * multiplier; }
            }
        }

        public static void Edit(ExcelWorksheet worksheet)
        {
            ExcelRange range = null; ExcelRange rows = null; ExcelRange columns = null; ExcelRange firstRow = null; ExcelRange cells = null;
            dynamic borders = null; dynamic interior = null; dynamic font = null;

            try
            {
                range = worksheet.UsedRange; rows = range.Rows; columns = range.Columns; firstRow = rows[1]; cells = range.Cells;
                ...
                borders = range.Borders; borders.LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous; borders.Weight = Microsoft.Office.Interop.Excel.XlBorderWeight.xlThin; borders.Color = (int)Microsoft.Office.Interop.Excel.XlRgbColor.rgbBlack;

                firstRow.RowHeight = 25; interior = firstRow.Interior; interior.Color = (int)Microsoft.Office.Interop.Excel.XlRgbColor.rgbLightGray; font = firstRow.Font; font.Bold = true;

                int colCount = columns.Count; columns.AutoFit();
            }
            finally
            {
                CoreUtils.ReleaseCom(ref font); CoreUtils.ReleaseCom(ref interior); CoreUtils.ReleaseCom(ref borders);
                CoreUtils.ReleaseCom(ref cells); CoreUtils.ReleaseCom(ref firstRow); CoreUtils.ReleaseCom(ref columns); CoreUtils.ReleaseCom(ref rows); CoreUtils.ReleaseCom(ref range);
            }
        }

        public static string GetValue(ExcelRange range, int row, int column)
        {
            ExcelRange cells = null; string value = null;
        ...
        {
            ExcelFormatCondition rule = null; dynamic interior = null;

            try
            {
                string formula = $"={address}=\"{criteria}\""; rule = (ExcelFormatCondition)conditions.Add(Microsoft.Office.Interop.Excel.XlFormatConditionType.xlExpression, Formula1: formula);

                interior = rule.Interior; interior.Color = ColorTranslator.ToOle(color); rule.StopIfTrue = false;
            }
            finally { CoreUtils.ReleaseCom(ref interior); CoreUtils.ReleaseCom(ref rule); }
        }
    }
}