namespace SolidEdgeAdd_In.Utils
{
    public class RaportGenerationUtils
    {
        public static void DxfsMemory(string location, object[,] data, int typeNumber, int nameNumber, int checkDxfColumn, int rowCount)
        {
            ...
            var dxfFileNames = allDxfFiles.Select(f => new
            {
                Path = f,
                Name = Path.GetFileName(f)
            }).ToList();

            var cadFileNames = allCadFiles.Select(f => new
            {
                Path = f,
                Name = Path.GetFileName(f)
            }).ToList();
                ...
                if (type.Equals(Constants.Styles.SheetMetal, StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (hasDxf)
                    {
                        ...
                        if (!string.IsNullOrEmpty(cadFilePath))
                        {
                            ...
                            {
                                data[i, checkDxfColumn] = dxfDate;
                            }
                            else
                            {
                                data[i, checkDxfColumn] = "Missing DXF Property";
                            }
                        }
                    }
                    else
                    {
                        data[i, checkDxfColumn] = "Missing DXF";
                    }
                }
                else
                {
            ...
            try
            {
                ...
                for (int i = 1; i <= rowCount; i++)
                {
                    ...
                    try
                    {
                        ...
                        cells = row.Cells;
                        shotCell = (ExcelRange)cells[1, imageNumber];
                    ...
                    {
                        CoreUtils.ReleaseCom(ref cmdBars);
                        CoreUtils.ReleaseCom(ref excelApp);
                        CoreUtils.ReleaseCom(ref shotCell);
