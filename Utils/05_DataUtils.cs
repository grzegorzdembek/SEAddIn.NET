namespace SolidEdgeAdd_In.Utils
{
    public class DataUtils
    {
        ...
        public static void ClearDxfDates(SeOccurrences assemblyOccurrences, HashSet<string> processed, Logger logger)
        {
            ProcessDataForClearDxfDates(assemblyOccurrences, processed, new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase), logger);
        }

        private static void ProcessDataForClearDxfDates(SeOccurrences occurrences, HashSet<string> processed, Dictionary<string, bool> assemblyCache, Logger logger)
        {
            int count = occurrences.Count;
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                try
                {
                    ...
                    if (occurrence.IncludeInBom == false)
                    {
                        logger.LogSkip(fileName, "File excluded from BOM");
                        continue;
                    }
                    ...
                    {
                        logger.LogSkip(fileName, "File unavailable");
                        continue;
                    }

                    if (string.IsNullOrEmpty(filePath))
                    {
                        logger.LogSkip(fileName, "File does not exist on disk");
                        continue;
                    }
                    ...
                    if (!processed.Contains(filePath))
                    {
                        ...
                        if (document is SePart || document is SeSheetMetal)
                        {
                            if (properties.IsTypeB)
                            {
                                ...
                                {
                                    properties.ClearDxfDate();
                                    logger.LogSuccess($"{fileName} - Cleared DxfDate property");
                                }
                                else
                                {
                                    logger.LogSkip(fileName, "No DxfDate property to clear");
                                }
                            }
                            else
                            {
                                logger.LogSkip(fileName, "Not a Sheet Metal part (Type B)");
                            }
                        }
                    ...
                    if (isAssembly && assemblyCache.TryGetValue(filePath, out bool isTypeA) && isTypeA)
                    {
                        ...
                        {
                            subOccurrences = ((SeAssembly)document).Occurrences;
                            ProcessDataForClearDxfDates(subOccurrences, processed, assemblyCache, logger);
                        }
                        finally
                        {
                            Helpers.ReleaseCom(ref subOccurrences);
                ...
                {
                    logger.LogError(fileName, $"Error processing file: {ex.Message}");
                    continue;
                }
                finally
                {
        ...
        public static void RefreshTree(SeOccurrences assemblyOccurrences, HashSet<string> paths)
        {
            ProcessDataForRefreshTree(assemblyOccurrences, paths, new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase));
        }

        private static void ProcessDataForRefreshTree(SeOccurrences occurrences, HashSet<string> paths, Dictionary<string, bool> assemblyCache)
        {
            int count = occurrences.Count;
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                try
                {
                    ...
                    if (isAssembly && assemblyCache.TryGetValue(filePath, out bool isTypeA) && isTypeA)
                    {
                        ...
                        {
                            subOccurrences = ((SeAssembly)document).Occurrences;
                            ProcessDataForRefreshTree(subOccurrences, paths, assemblyCache);
                        }
                        finally
                        {
                            Helpers.ReleaseCom(ref subOccurrences);
        ...
        public static void BuildDataForCorrectModelsView(SeOccurrences assemblyOccurrences, HashSet<string> paths)
        {
            ProcessDataForCorrectModelsView(assemblyOccurrences, paths, new Dictionary<string, bool>(StringComparer.OrdinalIgnoreCase));
        }

        private static void ProcessDataForCorrectModelsView(SeOccurrences occurrences, HashSet<string> paths, Dictionary<string, bool> assemblyCache)
        {
            int count = occurrences.Count;
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                try
                {
                    ...
                    if (isAssembly && assemblyCache.TryGetValue(filePath, out bool isTypeA) && isTypeA)
                    {
                        ...
                        {
                            subOccurrences = ((SeAssembly)document).Occurrences;
                            ProcessDataForCorrectModelsView(subOccurrences, paths, assemblyCache);
                        }
                        finally
                        {
                            Helpers.ReleaseCom(ref subOccurrences);
