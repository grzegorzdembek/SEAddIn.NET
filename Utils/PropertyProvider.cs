namespace SolidEdgeAdd_In.Utils
{
    ...
    public class PropertyProvider : IDisposable
    {
        ...
        { 
            get
            { 
                object rawValue = _isFileMode ? GetCustomFileProperty(Constants.Properties.CustomSet, Constants.Properties.Quantity) : GetCustomDocProperty(Constants.Properties.CustomSet, Constants.Properties.Quantity);
                return (rawValue != null && int.TryParse(rawValue.ToString(), out int count)) ? count : 0;
            } 
            set => SetProperty(Constants.Properties.CustomSet, Constants.Properties.Quantity, value);
        }
        ...
        { 
            SetProperty(Constants.Properties.CustomSet, Constants.Properties.DxfDate, DateTime.Now.ToString("yyyy-MM-dd-HH-mm")); 
        }

        public void ClearDxfDate()
        { 
            SetProperty(Constants.Properties.CustomSet, Constants.Properties.DxfDate, String.Empty); 
        }

        public void SetSheetMaterial() 
        { 
            SetProperty(Constants.Properties.CustomSet, Constants.Properties.MaterialName, "Blachy"); 
        }

        private string GetPropertyString(string setName, string propName) 
        { 
            object rawValue = _isFileMode ? GetCustomFileProperty(setName, propName) : GetCustomDocProperty(setName, propName); return rawValue?.ToString(); 
        }
